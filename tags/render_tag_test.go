...
func TestRenderTag_ForLoopWithParams(t *testing.T) {
	...
	config.Cache[filepath.Join("testdata", "render_combined.html")] = []byte(`{{ item.name }} - {{ label }}`)
...
func TestRenderTag_DefaultAliases(t *testing.T) {
	config := render.NewConfig()
	loc := parser.SourceLoc{Pathname: filepath.Join("testdata", "render_source.html"), LineNo: 1}
	AddStandardTags(&config)

	config.Cache[filepath.Join("testdata", "product-card.liquid")] = []byte(`{{ product-card.name }}`)
	root, err := config.Compile(`{% render "product-card.liquid" with product %}`, loc)
	require.NoError(t, err)

	buf := new(bytes.Buffer)
	err = render.Render(root, buf, renderTestBindings, config)
	require.NoError(t, err)
	require.Equal(t, "Widget", buf.String())

	config.Cache[filepath.Join("testdata", "product-row.liquid")] = []byte(`{{ product-row.name }} `)
	root, err = config.Compile(`{% render "product-row.liquid" for products %}`, loc)
	require.NoError(t, err)

	buf.Reset()
	err = render.Render(root, buf, renderTestBindings, config)
	require.NoError(t, err)
	require.Equal(t, "Item1 Item2 Item3 ", buf.String())
}

func TestRenderTag_ForTypedSlice(t *testing.T) {
	config := render.NewConfig()
	loc := parser.SourceLoc{Pathname: filepath.Join("testdata", "render_source.html"), LineNo: 1}
	AddStandardTags(&config)
	config.Cache[filepath.Join("testdata", "typed.liquid")] = []byte(`{{ entry }}`)

	root, err := config.Compile(`{% render "typed.liquid" for values as entry %}`, loc)
	require.NoError(t, err)

	buf := new(bytes.Buffer)
	err = render.Render(root, buf, map[string]any{"values": []string{"a", "b", "c"}}, config)
	require.NoError(t, err)
	require.Equal(t, "abc", buf.String())
}

func TestRenderTag_ForEvaluatesParamsOnce(t *testing.T) {
	config := render.NewConfig()
	loc := parser.SourceLoc{Pathname: filepath.Join("testdata", "render_source.html"), LineNo: 1}
	AddStandardTags(&config)

	calls := 0
	config.AddFilter("tick", func(string) int {
		calls++
		return calls
	})
	config.Cache[filepath.Join("testdata", "counter.liquid")] = []byte(`{{ marker }}`)

	root, err := config.Compile(`{% render "counter.liquid" for values as value, marker: "" | tick %}`, loc)
	require.NoError(t, err)

	buf := new(bytes.Buffer)
	err = render.Render(root, buf, map[string]any{"values": []int{1, 2, 3}}, config)
	require.NoError(t, err)
	require.Equal(t, "111", buf.String())
	require.Equal(t, 1, calls)
}

func TestRenderTag_FilterWithCommaArgument(t *testing.T) {
	config := render.NewConfig()
	loc := parser.SourceLoc{Pathname: filepath.Join("testdata", "render_source.html"), LineNo: 1}
	AddStandardTags(&config)
	config.AddFilter("combine", func(value, left, right string) string {
		return value + left + right
	})
	config.Cache[filepath.Join("testdata", "filter-args.liquid")] = []byte(`{{ value }} {{ label }}`)

	root, err := config.Compile(
		`{% render "filter-args.liquid", value: "x" | combine: "y", "z", label: "ok" %}`,
		loc,
	)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	require.Equal(t, "xyz ok", buf.String())
}
...
func TestRenderTag_WithExpressionInParameters(t *testing.T) {
	...
	config.Cache[filepath.Join("testdata", "render_expr.html")] = []byte(`Count: {{ count }}, Total: {{ total }}`)
...
func TestRenderTag_ForloopVariables(t *testing.T) {
	...
	config.Cache[filepath.Join("testdata", "render_forloop.html")] = []byte(
...
func TestRenderTag_CachedTemplate(t *testing.T) {
	...
	config.Cache[filepath.Join("testdata", "cached_template.html")] = []byte("Cached: {{ value }}")
	...
	tests := []struct {
		...
		},
		{
			...
			checkFunc: func(t *testing.T, args *renderArgs) {
				...
				require.Equal(t, "key", args.params[0].name)
			},
		},
		{
			...
			checkFunc: func(t *testing.T, args *renderArgs) {
				...
				require.Equal(t, "key1", args.params[0].name)
				require.Equal(t, "key2", args.params[1].name)
			},
		},
		{
			name:  "quoted commas and escaped quotes",
			input: `"template.html", key: "a,\"b", other: value`,
			checkFunc: func(t *testing.T, args *renderArgs) {
				require.Len(t, args.params, 2)
				require.Equal(t, "key", args.params[0].name)
				require.Equal(t, "other", args.params[1].name)
			},
		},
		{
			name:  "with syntax",
